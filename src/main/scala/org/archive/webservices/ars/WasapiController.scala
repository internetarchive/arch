...
object WasapiController {
  ...
  def files(instance: DerivationJobInstance, downloadUrl: String, params: scalatra.Params, addSample: Boolean = true)(implicit request: HttpServletRequest): ActionResult = {
    val page = params
      .get("page")
    ...
    if (addSample && instance.conf.isSample) uriBuilder = uriBuilder.setParameter("sample", "true")
    Ok(
      ListMap(
        "count" -> count.asJson,
        "next" -> (if (page < pages)
        ...
        "files" -> files.map { file =>
          val locationUrl = downloadUrl + "/" + file.filename + (if (addSample && instance.conf.isSample)
              "?sample=true&access="
            else
      ...
      params.get("collection").flatMap(ArchCollection.get) match {
        case Some(collection) =>
          ...
          JobManager.getInstanceOrGlobal(
            ...
            case Some(instance) =>
              WasapiController.files(instance, ArchConf.baseUrl + "/files/download/" + collection.id + "/" + instance.job.id, params)
            case None =>
              NotFound()
          }
        case None =>
