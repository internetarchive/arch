...
class DefaultInputSpec(val specType: String, val cursor: HCursor) extends InputSpec {
  override lazy val id: String = cursor.get[String]("id").getOrElse(specType + ":" + cursor.focus.get.noSpaces.hashCode)
  override lazy val inputType: String = cursor.get[String]("inputType").getOrElse(InputSpec.InputType.Files)
...
object InputSpec {
  ...
  def apply(cursor: HCursor): InputSpec = {
    ...
    specType.filter(_ == CollectionBasedInputSpec.SpecType).flatMap { _ =>
      for {
  ...
  implicit def toCollectionBased(spec: InputSpec): CollectionBasedInputSpec = Try (
    spec.asInstanceOf[CollectionBasedInputSpec]
  ).getOrElse {
