...
class FileMetaData (val fields: Seq[FileMetaField]) extends Serializable {
  ...
  def str(key: String): Option[String] = map.get(key).flatMap(_.get[String]).orElse(strs(key).headOption)
  def strs(key: String): Seq[String] = map.get(key).toSeq.flatMap(_.gets[String])
  def int(key: String): Option[Int] = map.get(key).flatMap(_.get[Int]).orElse(ints(key).headOption)
...
object FileMetaData {
      ...
      Option(row.get(row.fieldIndex(field.name))).flatMap { value =>
        {
          field.dataType match {
            ...
            case array: ArrayType => array.elementType match {
  ...
  def fromJson(json: Json): FileMetaData = {
    ...
    val fields = cursor.keys.toSeq.flatten.flatMap { key =>
      cursor.downField(key).focus.map(key -> _)
    }.filter(!_._2.isNull).flatMap { case (k, v) =>
      {
        if (v.isString) v.asString.map(_ -> FileMetaFieldType.String)
          ...
          array.headOption.flatMap { head =>
            ...
            else if (head.isNumber) Some(array.flatMap(_.asNumber.map(_.toDouble)) -> FileMetaFieldType.Numbers)
            else if (head.isBoolean) Some(array.flatMap(_.asBoolean) -> FileMetaFieldType.Booleans)
            else None
          }
        }
        else None
