...
class CollectionAccessContext(
    val aitHdfsHostPort: Option[(String, Int)],
    val aitAuth: Option[String],
    val cachePath: String,
    val iaAuthHeader: Option[String] = None,
    alwaysAitHdfsIO: Boolean = false) {
  lazy val hdfsIO: HdfsIO = if (alwaysAitHdfsIO) aitHdfsIO else HdfsIO
  lazy val aitHdfsIO: HdfsIO =
...
object CollectionAccessContext {
  def fromArchConf: CollectionAccessContext =
    new CollectionAccessContext(
  ...
  def fromArchConf(alwaysAitHdfsIO: Boolean) =
    new CollectionAccessContext(
      aitHdfsHostPort = ArchConf.aitCollectionHdfsHostPort,
      aitAuth = ArchConf.foreignAitAuthHeader,
      cachePath = ArchConf.collectionCachePath,
