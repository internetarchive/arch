...
object AutRecordLoader {
  def fromWarc(filename: String, warc: WarcRecord, bufferBytes: Boolean = false): ArchiveRecord = new ArchiveRecord {
    private var bufferedPayload: Option[ByteArray] = None

    private var invalidated: Boolean = false
    private def payload: InputStream = {
      if (bufferBytes) {
        if (bufferedPayload.isEmpty) bufferedPayload = Some(MemoryCompressor.compress(warc.payload))
    ...
    def getCrawlMonth: String = warc.timestamp.filter(_.length >= 6).map(_.take(6)).getOrElse("")
