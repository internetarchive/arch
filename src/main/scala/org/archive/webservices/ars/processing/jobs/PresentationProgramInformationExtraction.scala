...
object PresentationProgramInformationExtraction extends ChainedJob {
  ...
  object Spark extends PartialDerivationJob(this) with SparkJob {
    ...
    def run(conf: DerivationJobConf): Future[Boolean] = {
      SparkJobManager.context.map { _ =>
        ...
        PresentationProgramInformationExtractor(df).write.option("timestampFormat", "yyyy/MM/dd HH:mm:ss ZZ").format("csv").csv(conf.outputPath + relativeOutPath)
        succeeded(conf)
      }
    }
  ...
  object PostProcessor extends PartialDerivationJob(this) with GenericJob {
    def run(conf: DerivationJobConf): Future[Boolean] = Future {
      ...
      Try {
        Seq("/bin/sh", "-c", "find " + outPath + " -iname 'part*' -type f -exec cat {} > " + outPath + "/presentation-program-information.csv \\;").! == 0 &&
        Seq("/bin/sh", "-c", "gzip " + outPath + "/presentation-program-information.csv").! == 0 &&
    ...
    override def history(conf: DerivationJobConf): DerivationJobInstance = {
      val instance = super.history(conf)
      val outPath = conf.outputPath + relativeOutPath
      if (HdfsIO.exists(outPath + "/presentation-program-information.csv.gz")) {
