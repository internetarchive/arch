...
trait ArchArchiveSparkFunctionAdapter[Source] {
  ...
  def noParams(on: Option[FieldPointer[ArchEnrichRoot[_], Source]]): EnrichFunc[ArchEnrichRoot[_], Source, _] = {
    val dependency = on.orElse(defaultDependency)
    dependency.map(baseFunc.on(_)).getOrElse(baseFunc).asInstanceOf[EnrichFunc[ArchEnrichRoot[_], Source, _]]
  }
  def noParams: EnrichFunc[ArchEnrichRoot[_], Source, _] = noParams(None)
  def withParams(params: DerivationJobParameters, on: Option[FieldPointer[ArchEnrichRoot[_], Source]] = None): EnrichFunc[ArchEnrichRoot[_], Source, _] = {
    if (params.isEmpty) noParams(on) else {
      val dependency = on.orElse(defaultDependency)
      val func = initFunc(params)
      dependency.map(func.on).getOrElse(func).asInstanceOf[EnrichFunc[ArchEnrichRoot[_], Source, _]]
    }
  }
  def initFunc(params: DerivationJobParameters): EnrichFunc[_, Source, _] = baseFunc
  def toDependencyPointer(func: EnrichFunc[ArchEnrichRoot[_], _, _]): Option[FieldPointer[ArchEnrichRoot[_], Source]] = Try {
    func.asInstanceOf[FieldPointer[ArchEnrichRoot[_], Source]]
  }.toOption
}

trait ArchArchiveSparkFunction[Source] extends ArchArchiveSparkFunctionAdapter[Source] { this: EnrichFunc[_, Source, _] =>
