...
import org.archive.webservices.archivespark.model.{Derivatives, EnrichFunc, EnrichRoot, TypedEnrichRoot, TypedEnrichable}
import org.archive.webservices.ars.model.collections.inputspecs.FileRecord
import org.archive.webservices.ars.processing.jobs.archivespark.base.{ArchFileRecord, LocalFileCache}
import org.archive.webservices.sparkling._
import org.archive.webservices.sparkling.io.{CleanupInputStream, IOUtil}

import scala.util.Try

object ArchFileBytes extends EnrichFunc[TypedEnrichRoot[FileRecord] with LocalFileCache, FileRecord, Array[Byte]] {
  ...
  override def derive(source: TypedEnrichable[FileRecord], derivatives: Derivatives): Unit = {
    val in = Try(source.asInstanceOf[LocalFileCache].localFileCache).toOption.flatten.getOrElse(source.get.access)
