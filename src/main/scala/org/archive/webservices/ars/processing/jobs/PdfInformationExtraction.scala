...
object PdfInformationExtraction extends ChainedJob {
  ...
  object Spark extends PartialDerivationJob(this) with SparkJob {
    ...
    def run(conf: DerivationJobConf): Future[Boolean] = {
      SparkJobManager.context.map { _ =>
        ...
        PDFInformationExtractor(df).write.option("timestampFormat", "yyyy/MM/dd HH:mm:ss ZZ").format("csv").csv(conf.outputPath + relativeOutPath)
        succeeded(conf)
      }
    }
  ...
  object PostProcessor extends PartialDerivationJob(this) with GenericJob {
    def run(conf: DerivationJobConf): Future[Boolean] = Future {
      ...
      Try {
        Seq("/bin/sh", "-c", "find " + outPath + " -iname 'part*' -type f -exec cat {} > " + outPath + "/pdf-information.csv \\;").! == 0 &&
