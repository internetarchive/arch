...
object PdfInformationExtraction extends ChainedJob {
  val name = "Extract PDF information"
  override val category = "Binary Information"
  override val description = "This will output a single file with the following columns: crawl date, URL of the PDF file, filename, PDF extension, MIME type as provided by the web server, MIME type as detected by Apache TIKA, PDF MD5 hash and PDF SHA1 hash."
    ...
    def run(conf: DerivationJobConf): Future[Boolean] = {
      SparkJobManager.context.map { _ =>
        val df = RddUtil
          ...
          .pdfs()
        PDFInformationExtractor(df).write
  ...
  object PostProcessor extends PartialDerivationJob(this) with GenericJob {
    def run(conf: DerivationJobConf): Future[Boolean] = Future {
      ...
      Try {
        Seq(
          "/bin/sh",
          "-c",
          "find " + outPath + " -iname 'part*' -type f -exec cat {} > " + outPath + "/pdf-information.csv \\;").! == 0 &&
        Seq("/bin/sh", "-c", "gzip " + outPath + "/pdf-information.csv").! == 0 &&
    ...
    override def history(conf: DerivationJobConf): DerivationJobInstance = {
      val instance = super.history(conf)
      val outPath = conf.outputPath + relativeOutPath
      if (HdfsIO.exists(outPath + "/pdf-information.csv.gz")) {
        instance.state =
